...
def generate_test_dataset():
	...
	if 'Dataset' not in os.listdir('..'):
		...
		os.mkdir('../Dataset/Validation_set')
	...
	KPs = []
	for i in range(int(num_images/4)):
		print('\rcreating Validation set : %i/%i'%(i+1, int(num_images/4)),end='')
		images,KP=dataset.__getitem__(index=i+(random.randint(0,3))*int(num_images/4))
		KPs.append(KP[0])
		cv2.imwrite('../Dataset/validation_set/' + str(i) + '.png',np.uint8(255*images[0]))
	np.save('../Dataset/validation_set_labels.npy', KPs)
	print('\rcreating Validation_set : --- done ---\n')

#generate_test_dataset()
#print(os.listdir('../IamStickman-main'))
...
def training_loop(DNN, head, epochs, optimizer, learning_rate, loss_fn, train_set, val_set, transform_labels, metric):
	...
	#print(val_set._len__())
	for step in range (val_set.__len__()):
		start_step = time.time()
		images, KP = train_set.__getitem__(index=step)
		val_model = DNN(images, training=False)
		loss = loss_fn(transform_labels(KP, val_model.shape[-2]), val_model)
		#loss_value += sum(DNN.losses)
		loss += loss
    #loss_value = loss_value/val_set.__len__()
	loss = loss_value/val_set.__len__()
		#loss_value *=learning_multiplier
	print("validation =")
	print(loss)
    #print("validation =")
